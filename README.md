...
1. Chiama `Diagnostics()` per vedere quali file sono stati trovati e da quale directory
...
I token DDL scadono in **60 secondi**. Se il token è scaduto, rilancia il corrispondente `Preview*` per ottenerne uno nuovo.

### `Operazione DDL non abilitata`

CREATE / ALTER / DROP sono disabilitati per default. Per abilitarli aggiorna `appsettings.json`:
...
Se Claude Code ha il MCP server in esecuzione, il build in Debug fallisce (exe in uso). Usa Release:
